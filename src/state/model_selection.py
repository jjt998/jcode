...
def resolve_model_snapshot(session: dict, profile: ModelProfile) -> dict:
    ...
    thinking_enabled = bool(selected.get("thinking_enabled", profile.thinking_enabled)) or profile.reasoning_always_on
    ...
    snapshot["reasoning_always_on"] = profile.reasoning_always_on
    return snapshot
...
def validate_model_options(profile: ModelProfile, thinking_enabled: bool, reasoning_effort: str) -> dict:
    ...
    if profile.reasoning_mode == "none":
        ...
        return {}
    if profile.reasoning_always_on:
